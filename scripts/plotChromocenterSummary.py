...
def plotSumCount(data, labels, title, out, ylim=None):
    ...
    for i, r in enumerate(p.patches):
        ...
        plt.text(r.get_x()+r.get_width()/2.0, height, np.isinf(b) and r'$0$' or
                (r'$%.2f%s$' %( v * 10**-b, int(b) and r'\cdot 10^{%.0f}'%b or
                    '')), ha='center', va=va, fontsize=12, color= data[i, 1] and
                GREEN or 'gray')
...
if __name__ == '__main__':
    ...
    for i, f in enumerate(args.stats):
        ...
        gc, chrx = basename(f.name).rsplit('_', 1)
        gc = gc.replace('_', ' ')
        labels.append(chrx.split(':', 1)[0])

    if args.ylim:
        plotSumCount(data, labels, gc, stdout, ylim=args.ylim)
    else:
        plotSumCount(data, labels, gc, stdout)
